.data 
buffer: .space 255			# "buffer" contendr√° la direcci√≥n base al string del archivo
file: .space 255 			# Direcci√≥n absoluta del archivo
...
optionText: .asciiz "Por favor, escoja una opci√≥n: "
...
mainMenu:
	addi $sp, $sp, -4	#Copia de seguridad de la direcci√≥n de la funci√≥n que llama
	sw $ra, 0($sp)
...
Up: 	lw $t2, cons1
	...
	add $t1, $s3, $s1	# Sumo mi iterador a la direccion base del string que contiene el archivo
	lb $t0, 0($t1)		# Cargo el i-esimo byte	en $t0
	beq $t0, $t2, loop	# Verifico si el prÛximo catacter leido es un salto de linea
	beq $t0, $t3, loop 	# Verifico si el prÛximo caracter leido es un espacio	
	beq $t0, $0,  esp	# Verifico si es un EOF
	addi $s4, $s4, 1	# Aumenta el contador de palabras en 1
	addi $t4, $t4, 1
	slt $t5, $s5,$t4  	# Verifico si excedo el n˙mero de caracteres por palabra
	beq $t5, $0, found 	# Branch a Found si es que mi palabra tiene m·s de "n" caracteres
